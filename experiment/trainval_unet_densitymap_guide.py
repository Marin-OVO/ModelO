"""
    train and val
        ...
        2026-01-30 experiment: dense map predictor
"""
import random
import numpy as np
...
from utils.engine.engine8th import train_one_epoch, val_one_epoch
...
def main(args):
    ...
    model = UNet1(in_channels=3, num_class=args.num_classes, bilinear=args.bilinear)
    model.to(device)
    ...
    train_end_transforms = [
        CustomTransformWrapper(
            FIDT(radius=args.radius, num_classes=args.num_classes, down_ratio=1),
    ...
    for epoch in range(last_epoch, args.epoch): # (0 - 149)
        ...
        if len(tmp_results['gt']) != 0 and len(tmp_results['dense']) != 0 and (epoch + 1) % 5 == 0:
            from utils.plot_curve import plot_dense
            plot_dense(tmp_results['gt'], tmp_results['dense'], work_dir, name=f"epoch_{epoch + 1:03d}")
